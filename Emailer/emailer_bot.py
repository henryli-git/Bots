...
def emailer(*args):
    ...
    while True:
        ...
        if send_attachments == 'y':
            attachments = [x.strip() for x in input("Paths of Attachments (use commas to separate paths): ").split(',')]
            for file in attachments:
                with open(file, 'rb') as f:
                    msg.add_attachment(f.read(), filename=f.name, maintype='application', subtype='octet-stream')
            break
